...
class OpeniocToCybox(object):
    """Converts OpenIOC content into CybOX Objects"""

    def __init__(self, namespace='http://example.com', prefix='example', verbose_mode=False):
    ...
    def get_indicatoritem_string(self, indicatoritem, separator = None):
    ...
    def map_condition_keywords(self, condition):
        condition_dict = {
            'is':'Equals',
            'isnot':'DoesNotEqual',
            'contains':'Contains',
    ...
    def process_indicator_item(self, indicator_item, observables = None, indicatoritem_dict = None):
        ...
        defined_object = ioc_observable.createObj(search_string, content_string, self.map_condition_keywords(condition))
        if defined_object != None:
            if observables != None:
                id_string = ''
                if indicator_item.get_id() is not None:
                    id_string = self.prefix + ':observable-' + self.normalize_id(indicator_item.get_id())
                else:
                    id_string = self.prefix + ':observable-' + self.generate_observable_id()
            ...
            if self.verbose_mode:
                ...
                if skipped_indicatoritem not in self.skipped_indicators:
                    self.skipped_indicators.append(skipped_indicatoritem)
            return False
        return

    #Test if an indicator is 'compatible', that is if it has at least one indicator item that is compatible with CybOX
    def test_compatible_indicator(self, indicator):
        for indicator_item in indicator.get_IndicatorItem():
            if self.process_indicator_item(indicator_item):
                return True
        #Recurse as needed to handle embedded indicators
        for embedded_indicator in indicator.get_Indicator():
            return self.test_compatible_indicator(embedded_indicator)
        
        return False

    #Process a single indicator and create the associated observable structure
    def process_indicator(self, indicator, observables, observable_composition, top_level=True):
        if self.test_compatible_indicator(indicator):
            #Dictionary for keeping track of indicatoritems without IDs
            indicatoritem_dict = {}
            current_composition = None
            if top_level == False:
                observable_id = self.prefix + ':observable-' + self.normalize_id(indicator.get_id())
            ...
            for indicator_item in indicator.get_IndicatorItem():
                if self.process_indicator_item(indicator_item, observables, indicatoritem_dict):
                    if indicator_item.get_id() is not None:
                        observable_idref = self.prefix + ':observable-' + self.normalize_id(indicator_item.get_id())
                        observable = cybox.ObservableType(idref=observable_idref)
                    else:
            ...
            #Recurse as needed to handle embedded indicators
            for embedded_indicator in indicator.get_Indicator():
                self.process_indicator(embedded_indicator, observables, current_composition, False)
        return

    #Generate CybOX output from the OpenIOC indicators
    def generate_cybox(self, indicators):
        ...
        for indicator in indicator_definition.get_Indicator():
            ...
            if indicator.get_id() is not None:
                ...
                id_string = self.prefix + ':indicator-' + self.generate_observable_id()
    ...
    #Helper methods
    def generate_observable_id(self):
        global obsv_id_base
        obsv_id_base += 1
...
def main():
    ...
    if os.path.isfile(infilename):    
        ...
        try:
            ...
            parser = OpeniocToCybox(verbose_mode=verbose_mode)
            observables = parser.generate_cybox(indicators)
 ...
 http://cybox.mitre.org/cybox_v1 http://cybox.mitre.org/XMLSchema/cybox_core_v1.0(draft).xsd"')
            if parser.verbose_mode:
                for indicator_id in parser.skipped_indicators:
